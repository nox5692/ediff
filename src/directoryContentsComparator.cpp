...
bool isFileAndDirectory( const std::string &path1, const std::string &path2 ) {
    ...
    return ( isPath1File && isPath2Directory ) || ( isPath1Directory && isPath2File );
}

directoryContentsComparator::directoryContentsComparator( const std::string &directory_1,
    ...
    for ( const auto &file : fs::directory_iterator( _name_1 ) )
        contents_1.push_back( file.path().string() ); // Can be file or directory
...
void directoryContentsComparator::findDifferences() {
    ...
    for ( auto item = _directory_1.second.begin(); item != _directory_1.second.end(); item++ ) {
        ...
        for ( auto counterpart = _directory_2.second.begin();
            ...
            if ( isFileAndDirectory( *item, *counterpart ) )
                continue;
            const char *items[] = { "", ( ( *item ).c_str() ), ( ( *counterpart ).c_str() ) };

            App app( 3, items );
            size_t comp_type;
            // By runnign getCompType() a directory comparator can be called again
        ...
        if ( !found_counterpart )
            // This means that one or more files have not found any matches -> directories are not equal
            _diff_count++;
    }
}
